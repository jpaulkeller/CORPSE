...
public final class ServerActivity 
{
	...
	private static final Pattern SERVER_PATTERN = Pattern.compile(
			"<h2 class=\"forumtitle\"><a href=\"([^\"&]+)[^\"]*?\">([A-Za-z]+)[^<]*</a></h2>", // .*?<li>Posts: ([0-9,]+)</li>", 
	...
	public void getServers(final String address, final Pattern pattern) 
	{
		...
		try 
		{
			...
			while (matcher.find()) 
			{
				...
				{
					String serverLink = "http://forums.lotro.com/" + matcher.group(1); 
	...
	// <dl class="threadlastpost td">...<dd>May 13 2012 <em class=\"time\">04:11 PM</em>"
	private static final Pattern LAST_POST_PATTERN = 
			Pattern.compile ("<dl class=\"threadlastpost td\">.*?<dd>([^<]+?) <em",
					Pattern.DOTALL | Pattern.MULTILINE);
	...
	public static void main(final String[] args) throws Exception
	{
		...
		/* test 1
		String serverName = "Landroval-EN-RE";
		app.servers.put(serverName, "http://forums.lotro.com/forumdisplay.php?81-Landroval-EN-RE"); 
		app.pages.put(serverName, 6);
		app.scrapeServer(serverName, firstDate);
		*/
		...
		if (app.byServer.size() > 1)
		{
			...
			Color[] gradient = Gradient.createMultiGradient(new Color[] {
					new Color(181, 32, 255), Color.blue, Color.green, Color.yellow, Color.red }, app.byServer.size());

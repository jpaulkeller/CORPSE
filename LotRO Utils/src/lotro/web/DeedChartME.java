...
public final class DeedChartME
{
   ...
   /*
   <tr>
   <td><b>Medal of Honor</b></td>
   <td>Complete 1 Mass Effect playthrough on any difficulty. Unlocks <a href="/wiki/Hardcore" title="Hardcore" class="mw-redirect">Hardcore</a> difficulty setting and unlocks character levels 51-60.</td>
   <td>100</td></tr>
   */
   ...
   public void loadDeeds(final String address) 
   {
      ...
      try 
      {
         ...
         while (matcher.find()) 
         {
            ...
            if (rqmt.startsWith("<"))
            {
               Matcher boldM = BOLD_PATTERN.matcher(rqmt);
               if (boldM.matches())
                  rqmt = boldM.group(1);
               else
                  System.err.println(name + " [" + rqmt + "]");
            }
            ...
            if (m2.matches())
            {
               ...
               System.out.println(name + "; " + word + " " + word2);
               if (word != null)
               {
                  if (word.toLowerCase().endsWith("ly")) // e.g. successfully
                     word = word2;
                  byWord.plus(word, 1);
               }
            }
   ...
   {
      DeedChartME app = new DeedChartME();

      // String url = "http://masseffect.wikia.com/wiki/Achievements";
      String url = "C:/Users/J/Desktop/Achievements.htm";

      String title = "Mass Effect Deeds by Type";
      app.loadDeeds(url);

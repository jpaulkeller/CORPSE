...
{
   private ComponentTools() { /* utility class */ }
   ...
   public static JFrame open (final String title, 
    ...
    * @param mnemonic - keyboard mnemonic character
    * @param iconFile - icon associated with the menu.
    */
   public static JMenu makeMenu (final String label, final char mnemonic, 
   ...
   /**
    * Create a JMenu (with icon, if provided) using the given arguments.
    * For this version, the mnemonic will be the first character of the label.
    * @param label    - menu text.
    * @param iconFile - icon associated with the menu.
    */
    ...
    * @param label    - text associated with the menu item.
    * @param command  - action to be performed when the menu item is
    *                   selected.
    * @param mnemonic - keyboard character mnemonic.
    * @param iconFile - icon to be displayed in the menu item.
    * @param tipText  - brief hint as to what the menu item does.
    * @param listener - action listener associated with the menu item.
    */
   public static JMenuItem makeMenuItem (final String label,
   ...
   /**
    * Create a JMenuItem (with icon, if provided) using the given arguments.
    * For this method, the actionCommand will be the same as the label.
    * @param label    - text associated with the menu item.
    * @param mnemonic - keyboard character mnemonic.
    * @param iconFile - icon to be displayed in the menu item.
    * @param tipText  - brief hint as to what the menu item does.
   ...
   public static JMenuItem makeMenuItem (final String label,
                                         ...
                                         final ActionListener listener)
   {
      return (makeMenuItem (label, label, mnemonic,
                            iconFile, tipText, listener));
   }

   /**
    * Create a JMenuItem (with icon, if provided) using the given arguments.
    * For this method, the actionCommand will be the same as the label,
    * and the mnemonic will be the first letter of the label.
    * @param label    - text associated with the menu item.
    * @param iconFile - icon to be displayed in the menu item.
    * @param tipText  - brief hint as to what the menu item does.
    * @param listener - action listener associated with the menu item.
   ...
   public static JMenuItem makeMenuItem (final String label,
                                         ...
                                         final ActionListener listener)
   {
      return (makeMenuItem (label, label, label.charAt (0),
                            iconFile, tipText, listener));
   }

   /**
    * Create a JCheckBoxMenuItem (with icon, if provided) using the
    * given arguments.
    * @param label    - text displayed with the JCheckBoxMenuItem.
    * @param mnemonic - keyboard mnemonic character.
    * @param iconFile - icon displayed with the JCheckBoxMenuItem.
    * @param tipText  - brief hint as to what the JCheckBoxMenuItem does.
    * @param state    - initial state.
    * @param listener - action listener.
    */
   public static JCheckBoxMenuItem makeCheckItem (final String label,
                                                  final char mnemonic,
                                                  final String iconFile,
                                                  final String tipText,
                                                  final boolean state,
   ...
   /**
    * Create a JCheckBoxMenuItem (with icon, if provided) using the
    * given arguments.  For this version, the mnemonic will be the
   ...
   public static JCheckBoxMenuItem makeCheckItem (final String label,
                                                  ...
                                                  final ActionListener listener)
   {
      return (makeCheckItem (label, label.charAt (0), iconFile, tipText,
                             state, listener));
   }

   /**
    * Create a JRadioButtonMenuItem (with icon, if provided) using the
    * given arguments.
    * @param group    - button group this button gets added to.
    * @param label    - text displayed with this JRadioButtonMenuItem.
    * @param mnemonic - keyboard mnemonic character
    * @param iconFile - icon displayed with this JRadioButtonMenuItem.
    * @param tipText  - brief hint as to what this JRadioButtonMenuItem does.
    * @param state    - initial state.
    * @param listener - action listener.
    */
   public static JRadioButtonMenuItem makeRadioItem (final ButtonGroup group,
                                                     final String label,
                                                     final char mnemonic,
                                                     final String iconFile,
                                                     final String tipText,
   ...
   public static JRadioButtonMenuItem makeRadioItem (final ButtonGroup group,
                                                     ...
                                                     final ActionListener listener)
   {
      return (makeRadioItem (group, label, label.charAt (0), iconFile, tipText,
                             state, listener));
   }

   /**
    * Create a JButton using the given icon, tooltip, and listener.
    * @param command  - action command tied to this button.
    * @param iconFile - image icon to be displayed with this button
    * @param tipText  - brief textual hint as to what this button does.
    * @param listener - action listener tied to this button.
    */
   public static JButton makeTool (final String command,
                                   final String iconFile,
                                   final String tipText,
                                   final ActionListener listener)
   {
   ...
   public static JButton makeButton (final String label, final String icon, 
   ...
   public static JButton makeButtonNarrow (final String label, final String icon, 
                                           final boolean enabled,
                                           final ActionListener listener,
                                           final String tip)
   {
   ...
   /**
    * Create a JToggleButton using the given icon, tooltip, and
    * listener.
    *
    * @param command  - action command tied to this button.
    * @param iconFile - image icon to be displayed with this button
    * @param tipText  - brief textual hint as to what this button does.
    * @param state    - initial selection state of this button.
    * @param listener - action listener tied to this button.
    */
   public static JToggleButton makeToggleTool (final String command,
                                               final String iconFile,
                                               final String tipText,
                                               final boolean state,
                                               final ActionListener listener)
   ...
   /**
    * Returns the best upper-left point at which to display the popup
    * component, near the given target component.  If target is null,
    * Point(0, 0) is returned.
    *
    * NOTE: This only works if target is currently displayed. Also,
    * note that the point returned is in screen coordinates (which
    * works well with the PopupFactory, but not with the Component
    * setLocation() method.  You may want to use moveNear() instead.
    */
   public static Point findBestLocationFor (final Component target, 
                                            final Component popup)
   {
      Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
      ...
      if (width > rightSpace && width > leftSpace &&
          height > topSpace && height > bottomSpace)
         return point;
   ...
   /**
    * Positions the given popup component adjacent to the given
    * target component, so that the popup will be fully on screen (if
    * possible). Note that the given target does not have to be the
    * actual parent containing the popup.
    */
    ...
    * @param comp      - component to shrink.
    * @param maxWidth  - maximum possible width of the component.
    * @param maxHeight - maximum possible height of the component.
    */
   public static void shrink (final Component comp, 
                              final int maxWidth, final int maxHeight)
   {
      Dimension dim = comp.getSize();
      comp.setSize (Math.min (dim.width, maxWidth),
                    Math.min (dim.height, maxHeight));
   }

   /** Creates a panel containing of buttons with the given labels. */

   public static JPanel createButtonPanel (final ActionListener listener,
   ...
   public static Font getFontBold (final Component c)
   {
      Font f = c.getFont();
      return f.isBold() ? f : 
         new Font (f.getName(), f.getStyle() + Font.BOLD, f.getSize());
   }
   ...
   /**
    * Convenience method for putting the component inside a panel with
    * a titled border (using the given title).
    */
   ...
   /**
    * Packs the given component's frame (or internal frame).  Useful
    * if the component changes size and does not have a reference to
   ...
   /**
    * Closes the given component's frame (or internal frame), and
    * disposes all of its resources.
    */
   ...
   public static void showProperties (final Component owner)
   {
      ...
      JOptionPane.showMessageDialog (owner, scroll, "System Properties",
                                     JOptionPane.INFORMATION_MESSAGE);
   }
   
   private static void appendProperty (final StringBuilder sb,
   ...
   private static void appendPropertyList (final StringBuilder sb,
                                           final String property)
   {
      sb.append (property);
      sb.append ("\n");

...
//TBD: handle fixed-width data (split at positions)
...
public class FieldPanel extends WizardPanel implements ItemListener
{
   ...
   public FieldPanel (final File source,
                      final Wizard wiz, 
                      ...
                      final Method onNextMethod)
   {
      ...
      headerFlag.setToolTipText
         ("Turn on if the first line of your data contains column names");
      ...
      // TBD: try to determine the actual delimiter, and setValue here
      // TBD: assume "," for *.CSV files
      ...
      trim.setToolTipText
         ("Remove leading and trailing white space from each value");
      trim.addItemListener (this);
      
      // TBD: flag to auto-trim quotes
      
      // TBD: flag to auto-strip HTML tags
   ...
   private void determineColumnCount (final String line)
   {
      ...
      if (columnCount == 0) // determine # of columns based on first row
      {
         ...
         /*
         spinner.removeValueChangeListener (spinnerListener);
         spinner.setValue (columnCount);
         spinner.addValueChangeListener (spinnerListener);
         */
      }
   }

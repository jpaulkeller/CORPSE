...
public class MapMaker implements Observer
{
   ...
   public static final String IMAGE_ROOT = "D:/pkgs/workspace/geoquest/icons";
   ...
   private void buildGUI()
   {
      ...
      terrainPalette = new DynamicPalette
         ("Terrain (bottom layer)", IMAGE_ROOT, "terrain", new IconButtonListener (0));
      terrainPalette.getPanel().setPreferredSize (new Dimension (200, 200));
      
      featurePalette = new DynamicPalette
         ("Features (top layer)", IMAGE_ROOT, "features", new IconButtonListener (1));
      ...
      palettes.add (featurePalette.getPanel(), JSplitPane.BOTTOM);
      
      JPanel left = new JPanel (new BorderLayout());
      left.add (palettes, BorderLayout.CENTER);
      left.add (menus.getModeBar(), BorderLayout.EAST);
   ...
   public void editProperties()
   {
      ...
      if (result == JOptionPane.OK_OPTION)
      {
         for (String key : map.getProps().keySet())
            System.out.println (key + " = " + map.getProps().get (key)); // TBD

...
public class Geocache implements Card, Comparable<Geocache>
{
   ...
   static final Map<String, Geocache> CACHES = new TreeMap<String, Geocache>();
   static { populate(); }
   ...
   public Geocache (final String name, final String type,
                    final int diff, final String text)
   {
      this.name = name;
      this.type = type;
      this.diff = diff;
...
--- CACHES
      ...
      // TBD unknown
      add ("Garden State Dark Way", null, 3, "Find requires one blue die");
      add ("Tycho Anomaly", null, 4, null);
...
BookCrossing Meets Geocaching
The Adirondack Traveler
Fairy Stones
Museum of Modern Artifacts
Cold Mountain
Heartbreak
Illuminati
Santa's Workshop
For Whom the Bell Tolls
Claustrophobia Met Arachnophobia
Triple Dog Dare You
*/
   
   private static void add (final String name, final String type,
                            final int diff, final String text)
   {
   ...
   private static void printCard (final PrintWriter out, final Geocache cache, 
                                  final int i)
   {
      if (i % PER_PAGE == 0) out.println ("<table cellspacing=12>\n");
      if (i % PER_ROW == 0) out.println ("<tr>");

      out.print ("<td style=\"" +
                 "background-color: " + cache.color + "; " +
                 "background-image: url(images/rules/AmmoBox-" + cache.diff + ".png); " +
                 "\" align=center height=110 width=160>");
      ...
      if (i % PER_ROW == PER_ROW - 1) out.println ("</tr>\n");
      if (i % PER_PAGE == PER_PAGE - 1) out.println ("</table></td>\n<p><hr><p>\n");
   ...
   private static String getTypeText (final String type)
   {
      if (type.contains ("/"))
      {
         ...
         return getTypeText (type.substring (0, split)) + "<br />" +
                getTypeText (type.substring (split + 1));
      }
      else if (type.contains ("Multi"))

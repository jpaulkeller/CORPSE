...
public class Cacher implements Card, Comparable<Cacher>
{
   ...
   static final Map<String, Cacher> CACHERS = new TreeMap<String, Cacher>();
   static { populate(); }
   ...
   public Cacher (final String name, final String startingEquipment, 
                  final String text)
   {
   ...
   private static void populate()
   {
      ...
      add ("Collector Colin", "Geocoin", "Colin can carry any number of Travel Bugs; and he starts the game with an extra (random) Travel Bug.");
      ...
      add ("Fast Freddie", "Flashlight", "Freddie always moves first.  Whenever a new cache is placed on the board, Freddie immediately takes an extra turn.");
      add ("Grampa Gary", "Gaiters", "Gary takes his time; he gets -1 to all Move rolls, and +1 to all Search rolls."); 
      add ("Hunter Henry", "Hat", "Henry gets +1 when searching in Forest tiles.  Other players can't play events on Henry if he's on a Forest tile.");
      
      add ("Independent Isabel", "Insect Repellent", "Other players may only play events on Isabel if they are in the same map quadrant.");
      ...
      add ("Nosey Norman", "Mirror", "Whenever another player rolls <em class=roll>6</em> or higher, Norman may immediately move 1 tile.");
      ...
      add ("Quirky Quigly", "Map", "Quigly may hold any number of Event cards; and he starts the game with a (random) Event card.");
      ...
      add ("Wandering Wendy", "Waders", "Wendy finds her own path.  She never gets the +1 path bonus, but if she rolls a <em class=find>FIND</em> while moving, she may take an extra turn.");
      ...
      add ("Yuppie Yuri", "Yellow Jeep", "During his turn, Yuri may spend 3 points to draw an Equipment card.");
      // TODO
      add ("Zealous Zach", "1 (random) Equipment Card", "Zach gets +1 on all Move rolls, as long as he has at least two empty Equipment slots.");
   }
   ...
   private static void printCard (final PrintWriter out, final Cacher cacher, 
                                  final int i)
   {
      ...
      out.println ("</td></tr>");
      /*
      out.println ("  <tr><td height=130 width=250 align=center>" + cacher.text + "</td></tr>");
      out.println ("  <tr><td height=50 bgcolor=" + Equipment.COLOR + " align=center valign=top>" +
